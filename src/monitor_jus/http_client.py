"""Cliente HTTP com semáforo, retry e Retry-After."""
...
from monitor_jus.metrics import observe_latency
...
class RateLimitedClient:
    def __init__(
        ...
        *,
        ...
        self._sync_sem = asyncio.Semaphore  # placeholder type hint unused
        self._sync_slots = max_concurrency
        self._sync_active = 0
        self._lock = asyncio.Lock() if False else None  # noqa: keep simple
        import threading
            ...
            try:
                ...
                if resp.status_code in (401, 403):
                    raise FailedAuthentication(
                        f"{self.source} auth failed: {resp.status_code}"
                    )
    ...
    if not raw:
        ...
        for key in ("x-ratelimit-reset", "X-RateLimit-Reset"):
            if key in resp.headers:
                try:
                    val = float(resp.headers[key])
                    if val > 1_000_000_000:  # epoch
